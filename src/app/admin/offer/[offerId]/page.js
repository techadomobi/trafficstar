...
function OfferDetailsPage() {
  ...
  useEffect(() => {
    if (!offerId) {
      setError("Offer ID is missing from the URL.");
      setIsLoading(false);
      return;
    }
    ...
    const fetchOfferDetails = async (partners_Id, advertiserId) => {
      ...
      try {
        ...
        if (data.responseCode !== 200 || !data.responseResult) {
          throw new Error(data.responseMessage || "Failed to fetch offer details.");
        }
      ...
      return (
        <div className="flex items-center justify-center h-[calc(100vh-10rem)]">
          <div className="flex flex-col items-center gap-4 text-slate-500">
            ...
            <p className="text-lg font-semibold">Loading Offer Details...</p>
          </div>
        </div>
      );
    }
        ...
        <div className="flex items-center justify-center h-[calc(100vh-10rem)] p-8">
          <Card className="w-full max-w-lg text-center shadow-lg">
            <CardHeader>
              ...
              <CardTitle className="mt-4 text-2xl">Unable to Load Offer</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-slate-600">{error || "The requested offer could not be found."}</p>
            </CardContent>
          </Card>
        </div>
      );
      ...
      <Card className="max-w-8xl mx-auto shadow-lg shadow-slate-200/50">
        <CardHeader className="p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <CardDescription className="text-lg text-slate-600 mt-1">Offer ID: {offer.offerId}</CardDescription>
            </div>
            {/* <Button size="lg"><Edit className="mr-2 h-4 w-4" /> Edit Offer</Button> */}
          </div>
        </CardHeader>
        ...
        <CardContent className="p-6 md:p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            <div>
              <h3 className="text-xl font-semibold text-slate-800 mb-4 pb-2 border-b-2 border-orange-500/50">Offer Details</h3>
            ...
            {offer.offerKpi && (
              <div className="md:col-span-2 mt-4">
                <h3 className="text-xl font-semibold text-slate-800 mb-4 pb-2 border-b-2 border-green-500/50">Offer KPIs</h3>
                <div 
                  className="prose max-w-none bg-slate-50 p-4 rounded-lg"
            ...
            {offer.landingPage && offer.landingPage.length > 0 && (
              <div className="md:col-span-2 mt-4">
                <h3 className="text-xl font-semibold text-slate-800 mb-4 pb-2 border-b-2 border-orange-500/50">Landing Page Details</h3>
                  ...
                  {offer.event.map((evt, index) => (
                    <div key={evt._id || index} className="bg-slate-50 p-4 rounded-lg border-l-4 border-orange-500">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-slate-600">Event Value:</span>
...
export default withAuth(OfferDetailsPage);